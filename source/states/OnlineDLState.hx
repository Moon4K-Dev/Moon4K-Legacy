...
class OnlineDLState extends SwagState {
		...
		if (FlxG.mouse.justPressed) {
			for (item in beatmapGrid.members) {
				if (item.overlapsPoint(FlxG.mouse.getWorldPosition())) {
					...
					openSubState(new SongInfoSubState(mapInfo.name, mapInfo.download, mapInfo.desc, mapInfo.author, mapInfo.image));
					break;
				}
			}
		}
	...
	function updatePage():Void {
		...
		for (i in startIndex...endIndex) {
			var map = files[i];
			var item = new BeatmapItem(Std.int((i % 3) * 210 + 20), Std.int(Math.floor((i % pageSize) / 3) * 160 + 80), map.name, map.download, map.image, i);
