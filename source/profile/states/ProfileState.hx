...
class ProfileState extends SwagState {
    ...
    override public function create():Void {
        ...
        circle.graphics.beginGradientFill(LINEAR, 
        ...
        var inputWidth = 300;
        nameInput = new FlxInputText(
            centerX + (containerWidth - inputWidth) / 2,
            profilePicture.y + pfpSize + 20,
            inputWidth,
        ...
        changePfpButton = createStyledButton(
            centerX + containerWidth/2 - 220,
            buttonY,
            "Change Picture",
            onChangePicture
        );
    ...
    private function formatNumber(n:Int):String {
        if (n >= 1000000) return Std.string(Math.floor(n / 100000) / 10) + "M";
        if (n >= 1000) return Std.string(Math.floor(n / 100) / 10) + "K";
        return Std.string(n);
    }
